...
async def judge_battle_route(request: Request, photo_a: UploadFile = File(...), photo_b: UploadFile = File(...)):
    ...
    for contents in [contents_a, contents_b]:
        if len(contents) > 10 * 1024 * 1024:
            raise HTTPException(400, "Image too large")

    ext_map = {"image/jpeg": "jpg", "image/png": "png", "image/webp": "webp"}
    ext_a = ext_map[photo_a.content_type]
    ext_b = ext_map[photo_b.content_type]
    ...
    doc = {
        "id": bid,
        "mode": "battle",
        "image_a_name": filename_a,
        "image_b_name": filename_b,
        "image_a_url": f"/api/uploads/{filename_a}",
        "image_b_url": f"/api/uploads/{filename_b}",
        "winner": verdict.get("winner", "A"),
        "a_score": max(0, min(100, verdict.get("a_score", 50))),
        "b_score": max(0, min(100, verdict.get("b_score", 50))),
        "verdict_line": verdict.get("verdict_line", "mid battle")[:200],
        "reasoning": verdict.get("reasoning", "")[:1000],
        "user_id": user["id"] if user else None,
        "user_handle": user["handle"] if user else "anon",
        "created_at": datetime.now(timezone.utc).isoformat()
    ...
    if user:
        winner_score = doc["a_score"] if doc["winner"] == "A" else doc["b_score"]
        await db_update_one("users", {"id": user["id"]}, {
            "$inc": {"lifetime_aura": winner_score, "verdicts_count": 1}
        })
...
async def leaderboard(limit: int = 20):
    ...
    for u in users:
        ...
        out.append({
            "id": u["id"],
            "handle": u.get("handle", "anon"),
            "lifetime_aura": u.get("lifetime_aura", 0),
            "verdicts_count": u.get("verdicts_count", 0),
            "tier": tier_info["tier"],
            "progress": tier_info["progress"]
        })
    return out

@app.get("/api/history")
async def history(user: dict = Depends(get_current_user), limit: int = 50):
    verdicts = await db_find("verdicts", {"user_id": user["id"]}, sort=("created_at", -1), limit=limit)
    battles = await db_find("battles", {"user_id": user["id"]}, sort=("created_at", -1), limit=limit)
    return {"verdicts": verdicts, "battles": battles}
