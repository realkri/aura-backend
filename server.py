...
async def judge_battle_route(request: Request, photo_a: UploadFile = File(...), photo_b: UploadFile = File(...)):
    ...
    contents_a = await photo_a.read()
    contents_b = await photo_b.read()

    ext_a = {"image/jpeg": "jpg", "image/png": "png", "image/webp": "webp"}[photo_a.content_type]
    ext_b = {"image/jpeg": "jpg", "image/png": "png", "image/webp": "webp"}[photo_b.content_type]

    filename_a = f"{uuid.uuid4().hex}.{ext_a}"
    filename_b = f"{uuid.uuid4().hex}.{ext_b}"

    with open(UPLOAD_DIR / filename_a, "wb") as f:
        f.write(contents_a)
    with open(UPLOAD_DIR / filename_b, "wb") as f:
        f.write(contents_b)

    image_a_b64 = base64.b64encode(contents_a).decode()
    image_b_b64 = base64.b64encode(contents_b).decode()
    verdict = await judge_battle(image_a_b64, image_b_b64)
    ...
    doc = {
        ...
        "mode": "battle",
        "image_a": filename_a,
        "image_a_url": f"/api/uploads/{filename_a}",
        "image_b": filename_b,
        "image_b_url": f"/api/uploads/{filename_b}",
    ...
    await db_insert("battles", doc)
    return doc

# ============================================================
# VERDICT & LEADERBOARD
# ============================================================
@app.get("/api/verdict/{vid}")
async def get_verdict(vid: str):
    v = await db_find_one("verdicts", {"id": vid})
    if v:
        return v
    b = await db_find_one("battles", {"id": vid})
    if b:
        return b
    raise HTTPException(404, "Not found")

@app.get("/api/leaderboard")
async def leaderboard(board: str = "aura", limit: int = 50):
    limit = min(max(limit, 1), 100)
    items = await db_find("verdicts", sort=("aura_score" if board == "aura" else "cringe_score", -1), limit=limit)
    return items

@app.get("/api/me/verdicts")
async def my_verdicts(user: dict = Depends(get_current_user)):
    items = await db_find("verdicts", {"user_id": user["id"]}, sort=("created_at", -1), limit=50)
    return items
...
# ============================================================
@app.get("/api/ranks")
async def ranks_leaderboard(limit: int = 50):
    """Leaderboard of users ranked by their lifetime aura / tier"""
    limit = min(max(limit, 1), 100)
    users = await db_find("users", sort=("lifetime_aura", -1), limit=limit)
    result = []
    for u in users:
        tier_info = get_user_tier(u.get("lifetime_aura", 0))
        result.append({
            "handle": u["handle"],
            "lifetime_aura": u.get("lifetime_aura", 0),
            "lifetime_cringe": u.get("lifetime_cringe", 0),
            "verdicts_count": u.get("verdicts_count", 0),
            **tier_info
        })
    return result
...
# ============================================================
async def generate_share_card(v: dict, app_url: str, blurred: bool = False) -> bytes:
    """Generate a PNG share card image for social media previews"""
    try:
        ...
        try:
            ...
            font_lock = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 72)
        except:
            font_title = ImageFont.load_default()
            font_text = font_title
            font_small = font_title
            font_lock = font_title

        # Draw gradient bar at top
        for i in range(width):
            r = int(204 * (1 - i/width) + 255 * (i/width))
            g = int(255 * (1 - i/width) + 20 * (i/width))
            b = int(0 * (1 - i/width) + 147 * (i/width))
            draw.line([(i, 0), (i, 10)], fill=(r, g, b))

        if blurred:
            # BLURRED MODE — tease the content, hide details
            tier = v.get("photo_tier", "???")

            # Draw lock icon and "SIGN IN TO UNLOCK" text
            draw.text((width//2 - 200, 180), "🔒", fill='#CCFF00', font=font_lock)
            draw.text((width//2 - 280, 300), "SIGN IN TO UNLOCK", fill='#CCFF00', font=font_title)
            draw.text((width//2 - 250, 380), f"Tier: {tier} · Aura: ??? · Cringe: ???", fill='#555566', font=font_text)
            draw.text((width//2 - 200, 480), "AURA — The AI Scorekeeper", fill='#333344', font=font_small)

            # Apply heavy blur
            img = img.filter(ImageFilter.GaussianBlur(radius=8))

            # Redraw text on top (sharp)
            draw = ImageDraw.Draw(img)
            draw.text((width//2 - 200, 180), "🔒", fill='#CCFF00', font=font_lock)
            draw.text((width//2 - 280, 300), "SIGN IN TO UNLOCK", fill='#CCFF00', font=font_title)
            draw.text((width//2 - 250, 380), f"Tier: {tier} · Aura: ??? · Cringe: ???", fill='#555566', font=font_text)
            ...
            tier = v.get("photo_tier", "PRIME")
            draw.text((60, 50), tier, fill='#CCFF00', font=font_title)

            # Verdict line (wrap)
            verdict = v.get("verdict_line", "mid energy")
            words = verdict.split()
            lines = []
            current = ""
            for word in words:
            ...
            y = 140
            for line in lines:
                draw.text((60, y), f'"{line}"', fill='#FFFFFF', font=font_text)
                y += 55

            # Scores
            aura = v.get("aura_score", 0)
            cringe = v.get("cringe_score", 0)
            y += 40
            draw.text((60, y), f"AURA: {aura}", fill='#CCFF00', font=font_title)
            draw.text((450, y), f"CRINGE: {cringe}", fill='#FF1493', font=font_title)

            # Branding
            y += 100
            draw.text((60, y), "AURA — The AI Scorekeeper for Real Life", fill='#555566', font=font_small)
...
<meta property="og:title" content="My AURA Verdict — {tier}">
<meta property="og:description" content="{verdict_line}">
<meta property="og:image" content="{og_image}">
<meta name="twitter:card" content="summary_large_image">
<title>AURA Verdict</title>
<style>
body {{ margin:0; background:#050505; color:#fff; font-family:'Outfit',sans-serif; display:flex; justify-content:center; align-items:center; min-height:100vh; }}
.card {{ background:#0a0a0f; border:1px solid #1a1a2e; max-width:500px; width:100%; padding:2rem; text-align:center; position:relative; }}
.card::before {{ content:''; position:absolute; top:-2px; left:-2px; right:-2px; height:4px; background:linear-gradient(90deg,#CCFF00,#FF1493); }}
.card img {{ width:100%; max-height:350px; object-fit:contain; margin-bottom:1.5rem; border:1px solid #1a1a2e; }}
.tier {{ font-family:'Unbounded',sans-serif; font-size:1.5rem; color:#CCFF00; letter-spacing:4px; margin-bottom:0.5rem; }}
.line {{ font-family:'Unbounded',sans-serif; font-size:1.1rem; margin-bottom:1.5rem; line-height:1.4; }}
.scores {{ display:flex; justify-content:center; gap:2rem; margin-bottom:1.5rem; }}
.score {{ text-align:center; }}
.score .num {{ font-family:'Unbounded',sans-serif; font-size:2.5rem; }}
.score .label {{ font-size:0.7rem; color:#555566; letter-spacing:2px; text-transform:uppercase; }}
.aura {{ color:#CCFF00; }}
.cringe {{ color:#FF1493; }}
.reasoning {{ color:#888899; font-size:0.9rem; line-height:1.5; margin-bottom:1.5rem; font-style:italic; }}
.btn {{ display:inline-block; padding:1rem 2rem; background:#CCFF00; color:#050505; text-decoration:none; font-family:'Unbounded',sans-serif; font-weight:bold; text-transform:uppercase; letter-spacing:2px; font-size:0.8rem; }}
</style>
<link href="https://fonts.googleapis.com/css2?family=Unbounded:wght@400;700;900&family=Outfit:wght@300;400;500;600&display=swap" rel="stylesheet">
</head>
<body>
<div class="card">
<img src="{image_url}" alt="">
<div class="tier">{tier}</div>
<div class="line">{verdict_line}</div>
<div class="reasoning">{reasoning}</div>
<div class="scores">
<div class="score"><div class="num aura">{aura}</div><div class="label">AURA</div></div>
<div class="score"><div class="num cringe">{cringe}</div><div class="label">CRINGE</div></div>
</div>
<a class="btn" href="{app_url}">Get Your Own Verdict</a>
</div>
</body>
</html>
"""
...
# ============================================================
# BLURRED SHARE CARD (for non-logged-in users)
# ============================================================
BLURRED_SHARE_HTML = """
<!DOCTYPE html>
<html>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta property="og:title" content="Someone got judged by AURA">
<meta property="og:description" content="Log in to see the verdict">
<meta property="og:image" content="{og_image}">
<meta name="twitter:card" content="summary_large_image">
<title>AURA Verdict — Locked</title>
<style>
body {{ margin:0; background:#050505; color:#fff; font-family:'Outfit',sans-serif; display:flex; justify-content:center; align-items:center; min-height:100vh; }}
.card {{ background:#0a0a0f; border:1px solid #1a1a2e; max-width:500px; width:100%; padding:2rem; text-align:center; position:relative; }}
.card::before {{ content:''; position:absolute; top:-2px; left:-2px; right:-2px; height:4px; background:linear-gradient(90deg,#CCFF00,#FF1493); }}
.locked-img {{ width:100%; max-height:300px; object-fit:cover; filter:blur(20px) brightness(0.3); margin-bottom:1.5rem; border:1px solid #1a1a2e; }}
.lock-icon {{ font-size:3rem; margin-bottom:1rem; }}
.tier {{ font-family:'Unbounded',sans-serif; font-size:1.5rem; color:#CCFF00; letter-spacing:4px; margin-bottom:0.5rem; }}
.line {{ font-size:1rem; color:#888899; margin-bottom:1.5rem; line-height:1.4; }}
.btn {{ display:inline-block; padding:1rem 2rem; background:#CCFF00; color:#050505; text-decoration:none; font-family:'Unbounded',sans-serif; font-weight:bold; text-transform:uppercase; letter-spacing:2px; font-size:0.8rem; margin:0.5rem; cursor:pointer; border:none; }}
.btn-outline {{ background:transparent; color:#CCFF00; border:2px solid #CCFF00; }}
</style>
<link href="https://fonts.googleapis.com/css2?family=Unbounded:wght@400;700;900&family=Outfit:wght@300;400;500;600&display=swap" rel="stylesheet">
</head>
<body>
<div class="card">
<img class="locked-img" src="{image_url}" alt="">
<div class="lock-icon">🔒</div>
<div class="tier">LOCKED</div>
<div class="line">This verdict is private.<br>Sign in or create an account to unlock it.</div>
<a class="btn" href="{app_url}">GET AURA</a>
<button class="btn btn-outline" onclick="window.location.href='{app_url}'">SIGN IN</button>
</div>
</body>
</html>
"""

@app.get("/api/share/{vid}")
async def share_card(vid: str, request: Request):
    v = await db_find_one("verdicts", {"id": vid})
    if not v:
        raise HTTPException(404, "Not found")

    # Auto-detect app URL
    host = request.headers.get("x-forwarded-host") or request.headers.get("host", "aura.app")
    ...
    # Generate blurred card for OG preview (always works, but blurred)
    card_bytes = await generate_share_card(v, app_url, blurred=True)
    og_image = image_url
    if card_bytes:
        og_image = f"{app_url}/api/share/{vid}/card.png"

    # If NOT logged in — show blurred/locked page
    if not user:
        html = BLURRED_SHARE_HTML.format(
            image_url=v.get("image_url", ""),
            og_image=og_image,
            app_url=app_url
        )
    ...
    # Logged in — show full card
    html = SHARE_HTML.format(
        tier=v.get("photo_tier", "PRIME"),
        verdict_line=v.get("verdict_line", ""),
        image_url=v.get("image_url", ""),
        og_image=og_image,
        aura=v.get("aura_score", 0),
        cringe=v.get("cringe_score", 0),
...
@app.get("/api/share/{vid}/card.png")
async def share_card_image(vid: str, request: Request):
    """Serve generated PNG share card — blurred for non-logged-in"""
    v = await db_find_one("verdicts", {"id": vid})
    if not v:
        raise HTTPException(404, "Not found")
    ...
    card_bytes = await generate_share_card(v, app_url, blurred=not user)
    if not card_bytes:
...
# PREMIUM STUB
# ============================================================
@app.post("/api/premium/stub")
async def premium_stub():
    return {"ok": False, "message": "Premium Verdict launches soon"}
...
@app.get("/")
async def health():
    return {"status": "ok", "app": "AURA", "db": "mongo" if DB else "memory", "version": "3.0"}
